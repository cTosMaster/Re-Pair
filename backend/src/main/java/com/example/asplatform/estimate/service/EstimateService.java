...
public class EstimateService {
     ...
     * - 타겟 수리요청은: 본인에게 배정 && 상태=WAITING_FOR_REPAIR && 미삭제
     * - 한 요청당 1개의 1차 견적만 허용(서비스 가드 + DB UNIQUE)
     * - 프리셋/수기 금액은 nullable 허용
     * - 총액(price) = COALESCE(preset_total,0) + COALESCE(manual_amount,0)
     * - 처리 완료 후 상태 전이: WAITING_FOR_REPAIR -> IN_PROGRESS (경합 안전)
     */
    @Transactional
    public EstimateCreateResponseDto createFirstEstimate(
            EstimateCreateRequestDto req,
            User user
    ) {
        ...
        RepairRequest target = repairRequestRepository
                ...
                .orElseThrow(() ->
                        new AccessDeniedException("대상 요청이 없거나 권한/상태가 올바르지 않습니다."));
        ...
        if (!presetIds.isEmpty()) {
            // 존재성/개수 검증
            long cnt = presetRepository.countByIdIn(presetIds);
            if (cnt != presetIds.size()) {
                throw new IllegalArgumentException("존재하지 않는 프리셋이 포함되어 있습니다.");
            }

            // 현재 Preset.price 합계 (서버 재계산)
            var presets = presetRepository.findAllById(presetIds);
        ...
        // 수기 금액(nullable 처리)
        Integer manual = (req.manualAmount() == null || req.manualAmount() == 0)
                ? null : req.manualAmount();
        ...
        repairStatusManager.changeStatus(
        	target.getRequestId(),
        ...
        return new EstimateCreateResponseDto(
                ...
                req.requestId(),
                presetTotal == null ? 0 : presetTotal,
                manual == null ? 0 : manual,
                total,
                presetIds.size(),
