...
public interface RepairRepository extends JpaRepository<Repair, Long> {
    ...
    @Query("SELECT r FROM Repair r " +
    	       "WHERE (r.request.user.customer.id = :customerId) " +
    	       "   OR (r.request.repairableItem.customer.id = :customerId)")
    	Page<Repair> findByCustomerId(@Param("customerId") Long customerId, Pageable pageable);
}
