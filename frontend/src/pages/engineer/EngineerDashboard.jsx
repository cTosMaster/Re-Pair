...
export default function EngineerDashboard() {
  ...
  /** ✅ 상세보기 시 engineerId 함께 전달 (쿼리 & state) */
  const goDetail = (row) => {
    const id = row?.id;
    if (!id) return;
    const seg = routeForStatus(row.statusUi);
    const q = row.engineerId ? `?eid=${encodeURIComponent(row.engineerId)}` : "";
    navigate(
      `/repair-requests/${encodeURIComponent(id)}/${seg}${q}`,
      row.engineerId ? { state: { engineerId: row.engineerId } } : undefined
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="px-6 text-xs text-gray-500 grid grid-cols-12">
        ...
        <div className="col-span-2 text-right">요청일자 / 상세보기</div>
      </div>
