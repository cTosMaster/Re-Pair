...
import { getMyRepairRequests } from '../../../services/userAPI'; // 경로 확인
...
export default function CompleteList() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      setTotalPages(tp);
      setHasNext(tp != null ? page + 1 < tp : content.length === PAGE_SIZE);
    } finally {
      setLoading(false);
    }
  }, [page, keyword, mapRow]);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">완료된 수리</h1>
              ...
              <div key={r.id} className="bg-white rounded-xl shadow-sm px-6 py-4">
                <div className="grid grid-cols-12 items-center">
                  ...
                  <div className="col-span-6">
                    <div className="font-semibold text-emerald-600">{r.title}</div>
                    {r.category && (
                      <span className="ml-1 text-xs text-gray-500">({r.category})</span>
                    )}
                  </div>
